...
class SkeletonDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(path).astype(np.float32)  # 讀取 .npy 檔案(骨架資料)
...
def train():
    ...
    # === 儲存最佳模型 ===
    torch.save(best_model_state, os.path.join(SAVE_DIR, f'stgcn_best_{LEARNING_RATE}_{EPOCHS}.pth'))
    print(f"\n最佳模型已儲存（Validation Acc: {best_val_acc:.2%}）為 stgcn_best_{LEARNING_RATE}_{EPOCHS}.pth")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, f"accuracy_plot_{LEARNING_RATE}_{EPOCHS}.png"))
