...
print("錄製中，請按下 G / R 開始錄製動作，按 Q 結束")
...
while cap.isOpened():
    ...
    joints = extract_joints(image_rgb,  results_hands)  # shape: [49, 3]
    joints = np.array(joints).T  # → [3, 49]

    # 鍵盤操作切換錄製的動作類別
    if key in [ord('g'), ord('r')] and not is_recording:
    ...
    if is_recording:
        ...
        cv2.putText(frame, f"Press G/R to start, X to stop", (20, 40),
