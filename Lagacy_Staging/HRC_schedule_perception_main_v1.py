"""
Server端 (機械手臂控制端URScript為Client)
HRC 作業監控主程式
...
def check_kitbox_replace(kitboxes, replace_idx):
    ...
    if inside_any and not sth_on_box:
        ...
        elif now - zone_stay_start[obj] >= required_stay_time + 0.7:
...
def should_trigger_delay(task, standard_time, now_from_start, cooldown=0.5):
    """
...
def classify_error(schedule, schedule_index, obj, human_current_task):
    ...
    - robot_executing  : 該物件是 robot task，且 robot 已經開始執行
    - robot_task       : 該物件是 robot task，但 robot 尚未開始
...
def wrong_item_message(error_class):
    messages = {
        "placed": "Wrong item : Already placed in this kit box",
        "robot_executing": "Wrong item : This task is being executed by robot",
        "robot_task": "Wrong item : This is a robot task item",
        "later_task": "Wrong item : Belongs to a later task",
        "not_belong": "Wrong item : Not belongs to this kit box",
    }
...
def  trigger_dynamic_reschedule_opp(current_idx, opp_done, now_from_start):
    ...
    global current_optimized_schedule, dynamic_summary_history
...
def handle_delay_failure(current_idx, now_from_start, standard_time):
    ...
    print(f"[DELAY FAILURE] task_id={task_id}, delay_time={delay_time:.2f}s")
    ...
    # print(f"[DELAY FAILURE] gantt saved: {info['gantt_path']}")
            ...
            if command == "replace":
                ...
                if kitbox_replace:
                    ...
                    if should_trigger_replace_delay(
                        ...
                        if info is not None:
                            print(f"Gantt saved: {os.path.basename(info['gantt_path'])}")
...
# 最終總覽：Planned timeline -> 每次 Dynamic Result -> Actual / Final Timeline 
summary_rows = [
    {
