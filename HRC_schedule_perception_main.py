"""
Client端 (URscript機械手臂控制端為Server)
...
def send_robot_task(task, schedule, robot_socket):
    """
    將任務資訊透過 TCP/IP 傳給 UR_task_control server
    """
    ...
    if robot_socket is None:
        print("[ROBOT TCP] Send task skipped: robot socket is None.")
        return False
    ...
    try:
        robot_socket.sendall(msg.encode("utf-8"))
        print(f"[ROBOT TCP] Sent task: {msg.strip()}")
    except OSError as e:
        print(f"[ROBOT TCP] Send task failed: {e}")
        return False
...
def send_kit_ready_to_robot(task_id, kit_ready):
    ...
    if client_socket is None:
        print("[ROBOT TCP] Send kit_ready skipped: robot socket is None.")
        return False
    ...
    try:
        ...
        print(f"[ROBOT TCP] Send kit_ready failed: {e}")
        return False
...
def dispatch_ready_robot_task(now_from_start):
    ...
    for i, task in enumerate(schedule):
        ...
        # 發送給 UR_task_control server
        send_success = send_robot_task(task, schedule, client_socket)
        if not send_success:
            return
...
def wrong_item_message(error_class):
    messages = {
        ...
        "robot_executing": "This task is being executed by robot",
        "robot_task": "This is a robot task item",
        "later_task": "Belongs to a later task",
        "not_belong": "Not belongs to this kit box",
    }
...
def reset_interrupted_delay_task(task_idx):
    ...
    print(
        f"[DELAY RESET] "
        f"interrupted delay task reset: task_id={tid}, "
    )
...
def  trigger_dynamic_reschedule_opp(current_idx, opp_done, now_from_start):
    ...
    global current_optimized_schedule, dynamic_summary_history, deferred_gantt_items, dynamic_info_records
    ...
    dynamic_info_records.append(deepcopy(info))
    ...
    print("[DYNAMIC RESCHEDULE] Opportunistic completion triggered.")

    return info

DELAY_FACTORS = [0.5, 1.0, 2.0]
...
def convert_failure_info_for_sim(info):
    """
    將監控端 info["failure_info"] 轉成 failure_simulation.py 需要的欄位

    failure_simulation.py 需要：
    - delay: type, t, delay_time
    - agent: type, t, new_agent
    - order: type, t1, t2
    """
    failure_info = info.get("failure_info", {})
    monitor_type = failure_info.get("type", "")
    ...
    if monitor_type == "delay":
        row["failure_type"] = "delay"
        row["t"] = (
            failure_info.get("delay_task_id")
            or failure_info.get("current_task_id")
            or ""
        )
        row["delay_time"] = failure_info.get("estimated_delay", "")
        return row

    if monitor_type == "agent_mistake":
        row["failure_type"] = "agent"

        completed_ids = failure_info.get("completed_task_ids", [])
        if completed_ids:
            row["t"] = completed_ids[0]

        # 監控端 agent_mistake 通常代表人員完成 robot 任務
        row["new_agent"] = "human"
        return row

    if monitor_type == "order_mistake":
        row["failure_type"] = "order"
        row["t1"] = failure_info.get("current_task_id", "")

        completed_ids = failure_info.get("completed_task_ids", [])
        if completed_ids:
            row["t2"] = completed_ids[0]

        return row

    # 其他事件目前不輸入 failure_simulation.py
    row["failure_type"] = monitor_type
    return row

def export_dynamic_events_csv(
        *,
        output_dir,
        dynamic_info_records,
    ):
    """
    匯出每次動態排程事件，作為模擬器的輸入參考 raw data
    """
    def ids_to_csv_text(ids):
        """
        將 id list 轉成 CSV 中容易讀取的格式
        """
        if not ids:
            return ""
    ...
    def task_ids_string(schedule_data):
        """
        將一份 schedule 轉成 task ID 順序字串
        """
        return " ".join(
            str(task.get("ID"))
    ...
    events_path = os.path.join(
        output_dir,
        f"dynamic_events_{timestamp}.csv"
    )
    ...
    with open(events_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for info in dynamic_info_records:
            sim_info = convert_failure_info_for_sim(info)

            dynamic_schedule = info["summary_item"]["schedule"]

            writer.writerow({
                "step_idx": info["deferred_gantt_item"].get("step_idx"),
                "failure_type": sim_info["failure_type"],
                "decision_time": info.get("decision_time", ""),
                "frozen_task_ids": ids_to_csv_text(info.get("frozen_task_ids", [])),
                "t": sim_info["t"],
                "t1": sim_info["t1"],
                "t2": sim_info["t2"],
                "new_agent": sim_info["new_agent"],
                "delay_time": sim_info["delay_time"],
                "dynamic_task_ids": task_ids_string(dynamic_schedule),
            })

    return events_path

def export_gantt_data_file(
        *,
        output_dir,
        offline_best_schedule,
        dynamic_info_records,
        final_schedule,
    ):
    """
    ...
    def write_gantt_block(f, title, rows, makespan):
    ...
    with open(gantt_path, "w", encoding="utf-8-sig") as f:
        # 離線最佳排程
        offline_rows, offline_makespan = export_monitor_timeline_rows(
            offline_best_schedule,
            actual_only=False,
        )
        ...
        write_gantt_block(
            ...
            rows=offline_rows,
            makespan=offline_makespan,
        )

        # 每次失效後動態排程結果
        for info in dynamic_info_records:
            step_idx = info.get("deferred_gantt_item", {}).get("step_idx", "")
            dynamic_schedule = info["summary_item"]["schedule"]

            dynamic_rows, dynamic_makespan = export_monitor_timeline_rows(
                dynamic_schedule,
                actual_only=False,
            )
            ...
            write_gantt_block(
                f,
                title=f"dynamic scheduling {step_idx}",
                rows=dynamic_rows,
                makespan=dynamic_makespan,
            )

        # 最後實際甘特圖
        final_rows, final_makespan = export_monitor_timeline_rows(
            final_schedule,
            actual_only=True,
        )
...
required_stay_time = 0.5  # 物件至少停留特定秒數才算完成任務
...
RADAR_WARNING_LOG_PREFIX = "RADAR_WARNING_LOG|"

radar_warning_intervals = []
radar_warning_log_received = False
...
UNITY_ERROR_TYPES = {
    # robot_task / later_task 目前只在 OpenCV 畫面顯示，不送 Unity
    "placed",
    "robot_executing",
    "not_belong",
    "wrong_zone",
...
def connect_unity():
    ...
    try:
        ...
        unity_socket = s
        print(f"[UNITY TCP] Connected to Unity server at {UNITY_HOST}:{UNITY_PORT}")
        return True

    except OSError as e:
        unity_socket = None
        print(f"[UNITY TCP] Connection failed: {e}")
        return False
...
def close_unity_connection():
    ...
    if unity_socket is not None:
        ...
        unity_socket = None
        print("[UNITY TCP] Connection closed.")
...
def send_to_unity(message):
    ...
    try:
        ...
        print(f"[UNITY TCP] Sent: {message.strip()}")
        return True

    except OSError as e:
        print(f"[UNITY TCP] Send failed: {e}")
...
def notify_current_human_task_to_unity(force=False):
    ...
    1. 若目前沒有 human task：
       - 若全部任務完成，送 COMPLETE
       - 若仍有未完成任務但目前沒有 human task，送 REPLACE_WAITING
    ...
    2. 若目前 human task 是 replace：
       ...
       - 否則送 REPLACE_WAITING，顯示等待該 kit box 任務完成
...
def start_task_monitoring(source="Unity"):
    ...
    if task_start:
        print("[TASK START] Task monitoring already started.")
        return

    # 如果區域點還沒取完，不要立刻開始
    if current_zone:
        pending_unity_start = True
        print("[TASK START] START received, but zones are not completed yet.")
        return
    ...
    print(f"[TASK START] Start task monitoring by {source}.")
...
def handle_radar_warning_log(message):
    """
    處理 Unity 回傳的倒車雷達警示區間

    格式：
    RADAR_WARNING_LOG|start,end;start,end
    """
    global radar_warning_intervals, radar_warning_log_received

    radar_warning_log_received = True
    radar_warning_intervals.clear()

    payload = message[len(RADAR_WARNING_LOG_PREFIX):].strip()

    print("[UNITY RADAR] Received radar warning log.")

    if not payload:
        print("[UNITY RADAR] No radar warning intervals.")
        return

    for idx, interval_text in enumerate(payload.split(";"), start=1):
        ...
        if len(parts) != 2:
            print(f"[UNITY RADAR] Invalid interval format: {interval_text}")
            continue
        ...
        try:
            ...
            print(f"[UNITY RADAR] Invalid interval value: {interval_text}")
            continue

        if end < start:
            print(f"[UNITY RADAR] Invalid interval range: {interval_text}")
            continue

        radar_warning_intervals.append({
            "start": start,
            "end": end,
            "duration": end - start,
        })
        ...
        # print(
        #     f"[UNITY RADAR] #{idx}: "
        #     f"start={start:.3f}s, end={end:.3f}s, duration={end - start:.3f}s"
        # )

    print(f"[UNITY RADAR] Total intervals: {len(radar_warning_intervals)}")

def handle_unity_message(message):
    """
    處理 Unity 傳來的文字訊息
    1. Unity Start Button ：START
    2. RADAR_WARNING_LOG|start,end;start,end
    3. 提示　OK:xxx
    """
    global monitor_start

    msg = message.strip()
    print(f"[UNITY TCP] Received: {msg}")

    if msg.startswith(RADAR_WARNING_LOG_PREFIX):
        handle_radar_warning_log(msg)
        return
...
# TCP/IP (client) to UR_task_control Server
# 若 UR_task_control.py 與監控主程式在同一台電腦
UR_SERVER_HOST = "127.0.0.1"
UR_SERVER_PORT = 5050

client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
client_socket.connect((UR_SERVER_HOST, UR_SERVER_PORT))
print(f"[ROBOT TCP] Connected to robot server at {UR_SERVER_HOST}:{UR_SERVER_PORT}")
        ...
        try:
            ...
            while "\n" in buffer:
                ...
                if msg.startswith("PICK_DONE|"):
                    ...
                    print(f"[ROBOT TCP] Received PICK_DONE: task_id={task_id}")

                elif msg.startswith("DONE|"):
                    task_id = int(msg.split("|")[1])
                    done_set.add(task_id)
                    robot_waiting_kit_ready_set.discard(task_id)
                    print(f"[ROBOT TCP] Received DONE: task_id={task_id}")

        except OSError as e:
            print(f"[ROBOT TCP] Receive failed: {e}")
            break
...
def close_robot_connection():
    ...
    if client_socket is not None:
        try:
            ...
            pass
        client_socket = None
        print("[ROBOT TCP] Connection closed.")
...
while cap.isOpened():
    ...
    # YOLO 推論
    results = model_yolo.predict(source=frame, conf=0.7, verbose=False)
        ...
        if human_current_task :
            ...
            if command == "replace":
                ...
                if opp_done:
                    ...
                    notify_current_human_task_to_unity(force=True)
                    opp_event_triggered = True
                
                # 檢查物件是否已經放置完成 or delay
                if not opp_event_triggered:
                    now = time.time()
                    ...
                    if target_object in zone_objects[target_zone]:
                        ...
                        # 物件離開正確 zone，重新計算 required_stay_time
                        if target_object in zone_stay_start:
                            del zone_stay_start[target_object]
                        
                    
                    # 只要任務尚未完成，就繼續判斷 delay 與 wrong zone
...
# 匯出動態排程事件 CSV 與甘特圖 raw data
dynamic_info_output_dir = os.path.join(BASE_DIR, "dynamic_raw_data_outputs")
events_csv_path = export_dynamic_events_csv(
    output_dir=dynamic_info_output_dir,
    dynamic_info_records=dynamic_info_records,
)
print(f"[DYNAMIC EVENTS CSV SAVED] {events_csv_path}")

gantt_data_path = export_gantt_data_file(
    output_dir=dynamic_info_output_dir,
    offline_best_schedule=offline_best_schedule,
    dynamic_info_records=dynamic_info_records,
    final_schedule=schedule,
)
print(f"[GANTT DATA SAVED] {gantt_data_path}")
...
summary_rows.append(
    {
        ...
        "radar_warning_intervals": deepcopy(radar_warning_intervals),
    }
)
